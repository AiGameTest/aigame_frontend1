...
export function AdminCaseEditPage() {
  ...
  const [baseStoryJson, setBaseStoryJson] = useState('');
      ...
      adminGetCase(numericId)
        .then((c) => {
          ...
          try {
            ...
            // baseStoryJson이 유효한 JSON이 아닐 경우 무시
          }
    ...
    adminGetUserCase(numericId)
      .then((c) => {
        ...
        try {
          ...
          setError('기존 scenarioPrompt가 JSON 형식이 아니어서 구조 편집으로 완전 복원하지 못했습니다.');
        }
      })
  ...
  return (
    <form className="max-w-4xl mx-auto space-y-5" onSubmit={onSubmit}>
      ...
      <div className="card space-y-3">
        ...
        <div className="flex items-center gap-3">
          {thumbnailUrl ? (
            <img src={thumbnailUrl} alt="썸네일" className="w-28 h-20 rounded-lg object-cover border border-white/10" />
          ) : (
            <div className="w-28 h-20 rounded-lg border border-dashed border-white/20 bg-zinc-900 flex items-center justify-center text-xs text-gray-500">
              미리보기
            </div>
          )}
          <div className="flex flex-col gap-1">
            ...
            {thumbnailUrl && (
              <button type="button" className="text-xs text-red-300 hover:text-red-200" onClick={() => setThumbnailUrl('')}>
                썸네일 삭제
              </button>
            )}
          </div>
